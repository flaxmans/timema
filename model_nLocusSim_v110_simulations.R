...
# 20 Sept. 2014                                           #
# A. A. Comeault                                          #
# simulation program and model name: "nLocusSim_v110"     #
# model writen by S. Flaxman                              #
###########################################################
#
#
# note that for this script to work properly,
# the nLocusSim_v110 program must be in your path.
#
#
...
############################################################
# Function to run the "nLocusSum_v110" simulation program: #
############################################################
run_nLocusSim <- function(base_dir, prefix, a, A, b, B, c, C, d, D, f, F, g, G, h, H, L, m, M, N, o, p, P, r, R, s, S, t, T, w, z, Z)
{ 
  ...
  # run program:
  system(paste("nLocusSim_v110", 
               "-a", a, "-A", A, "-b", b, "-B", B, "-c", c, "-C", C, "-d", d, "-D", D, "-f", f, "-F", F, "-g", g, "-G", G, "-h", h, "-H", H, "-L", L, "-m", m, "-M", M, "-N", N, "-o", o, "-p", p, "-P", P, "-r", r, "-R", R, "-s", s, "-S", S, "-t", t, "-T", T, "-w", w, "-z", z, "-Z", Z))
...
G_default <- 0.08        # MELANISTIC_MATING_ADVANTAGE_F
...
S_default <- 0.22        # S_COEFF_MELANISTIC_IN_GREEN_NICHE
t_default <- 0.11        # S_COEFF_GREEN_IN_DARK_NICHE
T_default <- 1000        # max NUMBER of GENERATIONS to run the simulation
w_default <- 0           # 0 = single run; 1 = multiple runs.
z_default <- 1           # ADDITIVE_MODEL: 1 is purely additive.
...
rs <- c(0.1)      # recombination rate

ms <- c(0.0133, 0.0200, 0.0266, 0.0400, 0.0600)   # sd of melanistic migration rate 
Ms <- c(0.0100, 0.0150, 0.0200, 0.0300, 0.0400)   # sd of green migration rate

Ss <- c(0.218, 0.218, 0.218, 0.216, 0.215)    # s against melanistic in green niche
ts <- c(0.15, 0.15, 0.15, 0.15, 0.15)         # s against green in dark niche

for (runs in seq_along(ms)) {
  # make a directory to write outputs for simulations using different migration rates and selection.
  system(paste("mkdir", paste("add_model", ms[runs], Ms[runs], sep="_")))
  ...
  for (i in 1:200) { # define the number of independent simulations to run (here we run 200).
    for (p in ps) {
      for (r in rs) {
        ...
        prefix <- paste( paste( "add_model", ms[runs], Ms[runs], sep="_"), '/', i, '_p', p, '_r', r, sep="" )
        ...
        run_nLocusSim(base_dir, prefix, a_default, A_default, b_default, B_default, c_default, C_default, d_default,
                      ...
                      ms[runs], Ms[runs],   # migration parameters (m and M) defined on lines 90 and 91.
                      N_default, o_default,
                      p, p, r,              # freq of melanistic (p, P) and recombination rate (r) parameters defined on lines 87 and 88
                      R_default, s_default, 
                      Ss[runs], ts[runs],   # selection parameters defined on lines 93 and 94
                      T_default, w_default, z_default, Z_default)
      }
    }
  }
